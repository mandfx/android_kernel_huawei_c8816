...
/*===========================================================================
 * FUNCTION    - bf3905_customize_config -
 * DESCRIPTION:  config sensor mirror/flip as product dts file
 *==========================================================================*/
void bf3905_customize_config(struct msm_sensor_ctrl_t *s_ctrl)
{
	int i=0, flip_mirror_value=0;

	if(0 == of_property_read_u32(
		s_ctrl->sensor_i2c_client->client->dev.of_node, 
		"qcom,flip-mirror-value", &flip_mirror_value))
	{
		CDBG("%s: flip-mirror-value=%d \n", __func__, flip_mirror_value);
		if(1 == flip_mirror_value)
		{
			CDBG("%s: need to flip&mirror \n", __func__);
			for(i=0; i<ARRAY_SIZE(bf3905_recommend_settings); i++)
			if (0x1e == bf3905_recommend_settings[i].reg_addr)
			{
				bf3905_recommend_settings[i].reg_data =  0x70;
				break;
			}
		}
	}
...
static int bf3905_add_project_name(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    bf3905_customize_config(s_ctrl);
