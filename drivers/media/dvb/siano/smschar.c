...
#define HOST_PRIV_RST_PIN   91
#define HOST_PRIV_PWR_EN    92
#define HOST_PRIV_PWN_LNA   6
...
int host_priv_rst_pin = HOST_PRIV_RST_PIN;
int host_priv_pwr_en  = HOST_PRIV_PWR_EN;    // 1.2v
int host_priv_pwr_lna = HOST_PRIV_PWN_LNA;   // 1.8v
//static struct regulator **priv_vdd;
				   ...
				   struct smschar_buffer_t *touser)
{
	...
	if (dev->cancel_waitq) {
		...
		sms_info("dev->cancel_waitq = true");
		return 0;
	}
		
	if (!dev->smsclient) {
		sms_err("no client: dev->smsclient == NULL");
		return -ENODEV;
	}
...
/*
static void smschar_pmic_control(bool8 onoff)
{
    if (onoff)
    {
    // turn on pmic l7
    priv_vdd = regulator_get();
    }
    else
    {
    // turn off pmic l7
...
static void smschar_poweron(void)
{
    // open the regulator, on QC's platform
    // smschar_pmic_control(1);

    if (0 > gpio_request(host_priv_pwr_en, "SMSPWREN"))
    {
        sms_err("Could not get GPIO for SMS POWER. Device will work but reset is not supported.\n");
        host_priv_pwr_en = 0xFFFFFFFF;

        goto SMSCHAR_POWERON_ERROR;;
    }
    if (0 > gpio_request(host_priv_pwr_lna, "SMSPWRLNA"))
    {
         sms_err("Could not get GPIO for SMS POWER LNA. Device will work but reset is not supported.\n");
         host_priv_pwr_lna = 0xFFFFFFFF;
        
         goto SMSCHAR_POWERON_ERROR;;
     }
     if (0 > gpio_direction_output(host_priv_pwr_en, 1))
    {
         sms_err("Could not set GPIO direction for SMS POWER. Device will work but reset is not supported.\n");
         gpio_free(host_priv_pwr_en);
         host_priv_pwr_en = 0xFFFFFFFF;

         goto SMSCHAR_POWERON_ERROR;
    }
    if (0 > gpio_direction_output(host_priv_pwr_lna, 1))
    {
        sms_err("Could not set GPIO direction for SMS POWER LNA. Device will work but reset is not supported.\n");
        gpio_free(host_priv_pwr_lna);
        host_priv_pwr_lna = 0xFFFFFFFF;

        goto SMSCHAR_POWERON_ERROR;
     }

      gpio_set_value(host_priv_pwr_en, 1);
    //note:1.2v should be at 0ms< t <=15ms earlier than 1.8v power on
    gpio_set_value(host_priv_pwr_lna, 1);
    sms_info("host_priv_pwr_en & host_priv_pwr_lna power on\n");
    

    msleep(10);

    if (0 > gpio_request(host_priv_rst_pin, "SMSRESET"))
    {
        sms_err("Could not get GPIO for SMS RESET. Device will work but reset is not supported.\n");
        gpio_free(host_priv_rst_pin);
        host_priv_rst_pin = 0xFFFFFFFF;
    ...
    else
    {
        ...
        if (0 > gpio_direction_output(host_priv_rst_pin, 0))
        {
        ...
        {
            gpio_set_value(host_priv_rst_pin, 0);
            sms_info("host_priv_rst power on");
        }
    }
    msleep(10);//10ms
    ...
    //gpio_set_value(host_priv_rst_pin, 0);
    msleep(10);
    gpio_set_value(host_priv_rst_pin, 1);
...
sms_info("sms power before sleep");
    msleep(200);
return;

SMSCHAR_POWERON_ERROR:
    //smschar_pmic_control(0); 
    sms_err("sms poweron exit unexpected");
}

static void smschar_poweroff(void)
{
    gpio_set_value(host_priv_rst_pin, 0);
    //note:1.2v should be at 0ms< t <=15ms later than 1.8v power off 
    msleep(1);
    gpio_set_value(host_priv_pwr_lna, 0);
    msleep(1);
    gpio_set_value(host_priv_pwr_en, 0);
    msleep(1);
    //smschar_pmic_control(0);  
