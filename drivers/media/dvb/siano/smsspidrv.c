...
#ifdef WORK_QUEUE
//sms_err("msg cpu 0x%x 0x%x 0x%x", ((struct thread_info *)(current->stack))->cpu,(current->pid),(current->tgid));
...
sms_info("sms sleep after send msg");
			msleep(30);
sms_info("sms sleep OK after send msg");

			ret = smsspi_check_version(spi_device);
			if (ret < 0)
			{
				sms_err("Device does not respond. release SPI bus, but this is not an error");
				// for loop modle to get version 2013 10 23 by terry

				//for (ret=0; ret<100; ret++)
				//{
				 //   msleep(1);
				//    smsspi_check_version(spi_device);
				//}
				ret = 0;
				goto device_not_found;
			}
...
{
	struct _spi_device_st *spi_device = spi_dev;//container_of(arg, struct _spi_device_st, connect_thread);
...
#ifdef WORK_QUEUE
//sms_err("int cpu 0x%x 0x%x 0x%x", ((struct thread_info *)(current->stack))->cpu,(current->pid),(current->tgid));
