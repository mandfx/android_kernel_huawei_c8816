...
{
    //nothing to do here, only wake up CPU
    return IRQ_HANDLED;
}

static bool if_nfc_support = false;
bool nfc_support(void)
{
    return if_nfc_support; 
}
			   ...
			   const struct i2c_device_id *id)
{
	...
	//turn on NFC clock when phone bootup
#ifdef CONFIG_HUAWEI_KERNEL
	nfc_clk_on();
#endif
	...
	//set clock req irq
	bcm2079x_dev->nfc_clk_irq = gpio_to_irq(platform_data->clock_req_gpio);
	retval = request_irq(bcm2079x_dev->nfc_clk_irq, bcm2079x_clkreq_irq_handler,
			IRQF_TRIGGER_RISING | IRQF_NO_SUSPEND, client->name, bcm2079x_dev);
	if (retval) {
		printk("set bcm2079x_clkreq_irq_failed\n");
		goto err_request_irq_failed;
	}
	enable_irq_wake(bcm2079x_dev->nfc_clk_irq);
