...
/* define BCD and ga value */
static uint8_t als_atime_tb[] = { 0xF6, 0xED, 0xDB }; /* 27.2ms,  51.68ms , 100.64ms */
static uint8_t als_again_tb[] = { 1, 8, 16, 120 };
static int lux_last = 300;
static int IAC_last = 0;
static uint32_t a_ga = 160;
static uint32_t c_ga = 500;
static uint32_t d_ga = 160;
#define APDS99XX_ALS_COE_B  186
#define APDS99XX_ALS_COE_C  75
#define APDS99XX_ALS_COE_D  129
#define APDS99XX_ALS_DF     52
...
/* modify luxcalculation add ga in different light */
static int luxcalculation(int cdata, int irdata)
{
    ...
    if(APDS99XX_CDATA_MAX < cdata)
    {
		/*If cdata is more than SNS_DD_APDS99XX_CDATA_MAX,then return the max lux*/
		luxValue = APDS99XX_LUX_MAX;
		return (luxValue);
    }

    IAC1 = (cdata - (APDS99XX_ALS_COE_B*irdata)/100);
    IAC2 = ((APDS99XX_ALS_COE_C*cdata)/100 - (APDS99XX_ALS_COE_D*irdata)/100);

    PROXIMITY_DEBUG("%s:IAC1 = %d,IAC2 = %d\n", _func_, IAC1, IAC2);
    ...
    /*change the range of different light*/
    /*In different color temprature light,we use different GA to calculate lux*/
    /*A light*/
    if(cdata < 220*irdata/100)
    {
    	
		luxValue = (IAC*a_ga*APDS99XX_ALS_DF)/100;
    }
    /*D light*/
    else if((cdata >= 220*irdata/100) && (cdata <= 43*irdata/10))
    {
		luxValue = (IAC*d_ga*APDS99XX_ALS_DF)/100;
    }
    /*C light*/
    else
    {
		luxValue = (IAC*c_ga*APDS99XX_ALS_DF)/100;
    }
    luxValue /= (272*(256-als_atime_tb[2])*als_again_tb[0]/100);
    if(luxValue < 0)
    {
		luxValue = lux_last;
		return (luxValue);
    ...
    luxValue = luxValue < APDS99XX_LUX_MAX ? luxValue : APDS99XX_LUX_MAX;
    return (luxValue);
}
...
Light_init:
    ...
    if (atomic_read(&l_flag) && (status & APDS9930_STATUS_ALS_BIT)) 
    {
        ...
        cdata_high = (cdata *  500)/500;
        cdata_low = (cdata *  500)/500;
        ...
        /* if lux<0,we need to change the gain which we can set register 0x0f */
                ...
                pr_err("%s, line %d: Need to change gain(lux=%d)\n", __func__, __LINE__, lux);
                light_data_value = APDS99XX_LUX_MAX;
                input_report_abs(aps->input_dev, ABS_LIGHT, APDS99XX_LUX_MAX);
                input_sync(aps->input_dev);
        }
    }   
