...
//modify:?迄?Y????DT??
#include <open_test_ANA1_truly.h>
#include <open_test_ANA2_truly.h>
#include <open_test_ANA1_B_truly.h>
#include <open_test_ANA2_B_truly.h>
#include <open_test_ANA3_truly.h>
///////////////////////////////////////////////////////////////////////////
u8 bItoTestDebug = 1;
...
static void ito_test_reset(void)//modify:?迄?Y????DT??
{
...
static void ito_test_disable_irq(void)//modify:?迄?Y????DT??
{
	msg2138_disable_irq();
}
static void ito_test_enable_irq(void)//modify:?迄?Y????DT??
{
	msg2138_enable_irq();
}
...
static u32 ito_test_choose_TpType(void)
{
    ...
    for(i=0;i<10;i++)
    {
        ...
        if(TP_OF_TRULY==tpType)//modify:℅⊿辰a??????tp那y??
        {
            break;
        }
        else if(i<5)
    ...
    {
        open_1 = open_1_truly;
        open_1B = open_1B_truly;
        open_2 = open_2_truly;
        open_2B = open_2B_truly;
        open_3 = open_3_truly;
        MAP1 = MAP1_truly;
        MAP2 = MAP2_truly;
        MAP3 = MAP3_truly;
        MAP40_1 = MAP40_1_truly;
        MAP40_2 = MAP40_2_truly;
        MAP40_3 = MAP40_3_truly;
        MAP40_4 = MAP40_4_truly;
        MAP41_1 = MAP41_1_truly;
        MAP41_2 = MAP41_2_truly;
        MAP41_3 = MAP41_3_truly;
        MAP41_4 = MAP41_4_truly;
        ito_test_keynum = NUM_KEY_TRULY;
        ito_test_dummynum = NUM_DUMMY_TRULY;
        ito_test_trianglenum = NUM_SENSOR_TRULY;
        ito_test_2r = ENABLE_2R_TRULY;
    }
    else
    {
        tpType = 0;
...
static ITO_TEST_RET ito_test_interface(void)
{
    ...
    qup_set_clk_freq(i2c_client->adapter, 50000);
...
static int msg_scap_test_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    bItoTestDebug = 1;
   error =  sprintf(buf, "[MSG]open debug log\n");
...
static int msg_scap_test_sample_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    {
        ITO_TEST_DEBUG_MUST("data_1[%d]=%d;\n",i,s16_raw_data_1[i]);
	  length = sprintf(tmp_buf, "data_1[%d]=%d;\n",i,s16_raw_data_1[i]);
	  strcat(data, tmp_buf);
    }
    mdelay(5);
    for(i=0;i<48;i++)
    {
        ITO_TEST_DEBUG_MUST("data_2[%d]=%d;\n",i,s16_raw_data_2[i]);
	  length = sprintf(tmp_buf, "data_2[%d]=%d;\n",i,s16_raw_data_2[i]);
	  strcat(data, tmp_buf);
    }
    mdelay(5);
    for(i=0;i<48;i++)
    {
        ITO_TEST_DEBUG_MUST("data_3[%d]=%d;\n",i,s16_raw_data_3[i]);
	 length = sprintf(tmp_buf, "data_3[%d]=%d;\n",i,s16_raw_data_3[i]);
	 strcat(data, tmp_buf);
    }
    mdelay(5);
	error = sprintf(buf, data);
	kfree(data);
