...
u16 max_limit_value = 3800;     // screen max limit
u16 min_limit_value = 500;     // screen min limit
u16 max_limit_key = 3500;       // key_val max limit
u16 min_limit_key = 500;        // key_val min limit
u16 uniformity_lmt = 100;        // screen uniformity in percent
...
s32 gtp_parse_config(struct i2c_client *client)
{
    ...
    tp_log_info("Driver num: %d, Sensor Num: %d", gt9xx_drv_num, gt9xx_sen_num);
    ...
    {
        chip_type_gt9f = 0;
        tp_log_info("Chip type: GT9XX");
    }
    else
    {
        chip_type_gt9f = 1;
        tp_log_info("Chip type: GT9XXF");
    }
...
load_dsp_again:
    ...
    tp_log_info("step 2: burn dsp_short code");
    ...
    // Short Threshold
    tp_log_debug(" Short Threshold: 10");
    opr_buf[0] = 0x88;
    opr_buf[1] = 0x04;  
    opr_buf[2] = 0;
    opr_buf[3] = 10;
    ...
    // ADC Read Delay
    tp_log_debug(" ADC Read Delay: 150");
    opr_buf[0] = 0x88;
    opr_buf[1] = 0x06;
    opr_buf[2] = (u8)(150 >> 8);
    opr_buf[3] = (u8)(150);
    ...
    tp_log_debug(" DiffCode Short Threshold: 20");
    opr_buf[0] = 0x88;
    opr_buf[1] = 0x51;
    opr_buf[2] = (u8)(20 >> 8);
    opr_buf[3] = (u8)(20);
    ...
    */
    ...
    tp_log_debug("short_flag = 0x%02X", opr_buf[2]);
    SET_INFO_LINE_INFO("");
...
s32 gtp_read_rawdata(struct i2c_client* client, u16* data, u16 rawdata_num)
{
    ...
    for(i=0,j = 0; i < ((gt9xx_drv_num*gt9xx_sen_num)*2); i+=2)
    {
		...
		if(rawdata_num==3){
			record_tp_capacitance( data[i/2]);
		}
    ...
    for (i = 0; i < gt9xx_sc_pxl_cnt; i++)
    {
        if (raw_buf[i] < min_limit_value)
        {
            ...
            if (new_flag)   // new one
            {
                ...
                tp_log_debug("[%d, %d]rawdata: %d, raw min limit: %d", driver, sensor, raw_buf[i], min_limit_value);
            }
            else
            {
                continue;
    ...
    for (i = 0; i < gt9xx_sc_pxl_cnt; i++)
    {
        if (raw_buf[i] > max_limit_value)
        {
            ...
            if (new_flag)   // new one
            {
                ...
                touchpad_sum[j].raw_val = raw_buf[i];
                touchpad_sum[j].times = 1;    
                tp_log_debug("[%d, %d]rawdata: %d, raw max limit: %d", driver, sensor, raw_buf[i], max_limit_value);
            }
            else
            {
                continue;
    ...
    for (i = gt9xx_sc_pxl_cnt; i < gt9xx_pixel_cnt; ++i)
    {
        ...
        if (raw_buf[i] > max_limit_key)
        {
            ...
            if (new_flag)   // new one
            {
                ...
                tp_log_debug("[%d, %d]key rawdata: %d, key max limit: %d", driver,sensor, raw_buf[i], max_limit_key);
            }
            else
            {
                continue;
...
s32 gt9xx_open_test(struct i2c_client * client)
{
    ...
    tp_log_info("Parsing configuration...");
    ...
    tp_log_info("Step 1: Send Rawdata Cmd");
    ...
    tp_log_info("Step 2: Sample Rawdata");
    for (i = 0; i < GTP_OPEN_SAMPLE_NUM; ++i)
    {   
        rslt_buf_idx = i;
    ...
    tp_log_info("Step 3: Analyse Result");
...
static ssize_t gtp_sysfs_opentest_show(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    tp_log_debug("gt9xx_open_test start line=%d\n",__LINE__);
    ...
    if(SUCCESS==ret)		
        ret = sprintf(buf, "%s\n%s\n", "PASS",g_touch_capacitance);
    else
        ret = sprintf(buf, "%s\n%s\n", "FAIL",g_touch_capacitance);
...
s32 gtp_test_sysfs_init(void)
{
    ...
    goodix_debug_kobj = kobject_create_and_add("touch_screen", NULL) ;
