...
#define UPDATE_FILE_PATH_2          "/system/etc/firmware/goodix_fw_g630.bin"
#define UPDATE_FILE_PATH_1          "/sdcard/goodix_fw_g630.bin"
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    u8  buf[16];
    u8  i;
    tp_log_info("%s start line=%d ",__func__,__LINE__);
    ...
    tp_log_debug("IC Hardware info:%02x%02x%02x%02x", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1],
    ...
    for(retry=0; retry<2; retry++)
    {
        ...
        {
            tp_log_info("The check sum in ic is error.");
            tp_log_info("The IC will be updated by force.");
            continue;
        }
        break;
    }
    tp_log_debug("IC force update flag:0x%x", update_msg.force_update);
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    tp_log_info("%s start line=%d ",__func__,__LINE__);
    tp_log_debug("FILE HARDWARE INFO:%02x%02x%02x%02x", fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2], fw_head->hw_info[3]);
    tp_log_debug("FILE PID:%s", fw_head->pid);
    tp_log_info("FILE VID:%04x", fw_head->vid);

    tp_log_debug("IC HARDWARE INFO:%02x%02x%02x%02x", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1],
                                                   update_msg.ic_fw_msg.hw_info[2], update_msg.ic_fw_msg.hw_info[3]);
    tp_log_debug("IC PID:%s", update_msg.ic_fw_msg.pid);
    tp_log_info("IC VID:%04x", update_msg.ic_fw_msg.vid);

    //First two conditions
    if ( !memcmp(fw_head->hw_info, update_msg.ic_fw_msg.hw_info, sizeof(update_msg.ic_fw_msg.hw_info)))
    {
        tp_log_debug("Get the same hardware info.");
		...
		tp_log_debug("%s pid_cmp_len=%d,line=%d",__func__,pid_cmp_len,__LINE__);
        if (pid_cmp_len < strlen(update_msg.ic_fw_msg.pid))
        {
            pid_cmp_len = strlen(update_msg.ic_fw_msg.pid);
			tp_log_info("%s pid_cmp_len=%d,line=%d",__func__,pid_cmp_len,__LINE__);
        }
            ...
            (!memcmp(fw_head->pid, "91XX", 4)))
        {
            ...
            //The third condition
            if (fw_head->vid != update_msg.ic_fw_msg.vid)	//modified for zg 
            {
                tp_log_info("Need enter update.");
                return SUCCESS;
            }
            tp_log_err("Don't meet the third condition.");
            tp_log_err("File VID <= Ic VID, update aborted!");
        }
        else
        {
            tp_log_err("File PID != Ic PID, update aborted!");
        }
    }
    else
    {
        tp_log_err("Different Hardware, update aborted!");
    }
    return FAIL;
}
...
static s8 gup_update_config(struct i2c_client *client)
{
    ...
    if(FAIL == ret)
    {
        tp_log_err("[update_cfg]Read product id & version id fail.");
        return FAIL;
    }
    pid[5] = '\0';
    tp_log_debug("update cfg get pid:%s", &pid[GTP_ADDR_LENGTH]);
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    if (path)
    {
        tp_log_debug("Update File path:%s, %d", path, strlen(path));
    ...
    else
    {
        ...
        {
            tp_log_err("INVALID header_fw_array, check your gt9xx_firmware.h file!");
            return FAIL;           
        }
...
static u8 gup_burn_fw_section(struct i2c_client *client, u8 *fw_section, u16 start_addr, u8 bank_cmd )
{
    ...
    if(FAIL == ret)
    {
        tp_log_err("[burn_fw_section]recall check 8k firmware fail.");
        return FAIL;
    }
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
    ...
    tp_log_debug("[burn_dsp_isp]Begin burn dsp isp---->>");
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
    ...
    tp_log_debug("[burn_fw_ss51]Begin burn ss51 firmware---->>");
    ...
    while(retry++ < 5)
    {
        ...
        {
            tp_log_info("[burn_fw_ss51]Alloc %dk byte memory success.", (FW_SECTION_LENGTH/1024));
            break;
        }
    }
    if(retry >= 5)
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
    ...
    tp_log_debug("[burn_fw_dsp]Begin burn dsp firmware---->>");
    //step1:alloc memory
    ...
    while(retry++ < 5)
    {
        ...
        {
            tp_log_info("[burn_fw_dsp]Alloc %dk byte memory success.", (FW_SECTION_LENGTH/1024));
            break;
        }
    }
    if(retry >= 5)
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
    ...
    tp_log_debug("[burn_fw_boot]Begin burn bootloader firmware---->>");
    ...
    while(retry++ < 5)
    {
        ...
        {
            tp_log_info("[burn_fw_boot]Alloc %dk byte memory success.", (FW_BOOT_LENGTH/1024));
            break;
        }
    }
    if(retry >= 5)
    ...
    if(FAIL == ret)
    {
        tp_log_err("[burn_fw_boot]load firmware dsp fail.");
    ...
    tp_log_debug("[burn_fw_boot]step5:release ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x04);                 //20121211
    if(ret <= 0)
    {
    ...
    tp_log_debug("[burn_fw_boot]step7:burn 2k bootloader firmware");
    ret = gup_burn_proc(client, fw_boot, 0x9000, FW_BOOT_LENGTH);
    if(FAIL == ret)
    {
        tp_log_err("[burn_fw_boot]burn fw_section fail.");
        goto exit_burn_fw_boot;
    }
    ...
    if(FAIL == ret)
    {
        tp_log_err("[burn_fw_boot]recall check 4k dsp firmware fail.");
        goto exit_burn_fw_boot;
    }
    
    //step9:enable download DSP code 
    tp_log_debug("[burn_fw_boot]step9:enable download DSP code ");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, 0x99);
    if(ret <= 0)
    {
        tp_log_err("[burn_fw_boot]enable download DSP code fail.");
        ret = FAIL;
        goto exit_burn_fw_boot;
    }
    
    //step10:release ss51 & hold dsp
    tp_log_debug("[burn_fw_boot]step10:release ss51 & hold dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x08);
    if(ret <= 0)
    {
        tp_log_err("[burn_fw_boot]release ss51 & hold dsp fail.");
        ret = FAIL;
        goto exit_burn_fw_boot;
    }
...
s32 gup_update_proc(void *dir)
{
    ...
    ts = i2c_get_clientdata(i2c_connect_client);
	/*read the firmware version before update firmware*/
...
file_fail:
    ...
    if (SUCCESS == update_ret)
    {
        ...
        /*after updating successfully ,get config version and set touch_panel info*/
        ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_CONFIG_DATA, &config_id, 1);
        if (ret == SUCCESS)
        {
            sprintf(touch_info,"goodix_gt915_boen.%c",config_id);
        ...
        tp_log_info("%s fw update success line=%d ",__func__,__LINE__);
        return SUCCESS;
    }
    else
    {
        show_len = 200;
        tp_log_info("%s fw update fail or no need to update=%d ",__func__,__LINE__);
        return FAIL;
    }
}
...
u8 gup_init_update_proc(struct goodix_ts_data *ts)
{
    ...
    tp_log_info("Ready to run update thread.");
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
    ...
    if (FAIL == ret)
    {
        tp_log_err("failed to get ic firmware info");
        _CLOSE_FILE(update_msg.file);
        return FAIL;
    }
...
static u8 gup_check_update_file_fl(struct i2c_client *client, st_fw_head* fw_head, char* path)
{
	...
	printk(KERN_ERR "zengguang  ffffffffffffffffffffffffff\n");
    ret = SUCCESS;
    ...
    printk(KERN_ERR "zengguang  ddddddddddddddddddddd\n");
    return ret;
}
