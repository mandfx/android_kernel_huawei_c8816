...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    if (ts->fixed_cfg)
    {
        tp_log_info("Ic fixed config, no config sent!");
        return 0;
    }
    else if (ts->pnl_init_error)
    {
        tp_log_info("Error occured in init_panel, no config sent");
        return 0;
    }
    
    tp_log_info("Driver send config.");
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    {               
        ...
        tp_log_debug("0x814B = 0x%02X", doze_buf[2]);
        if (ret > 0)
        {               
            if (doze_buf[2] == 0xAA)
            {
                tp_log_info("Forward slide to light up the screen!");
            ...
            {
                tp_log_info("Backward slide to light up the screen!");
                doze_status = DOZE_WAKEUP;
                input_report_key(ts->input_dev, KEY_POWER, 1);
                input_sync(ts->input_dev);
                input_report_key(ts->input_dev, KEY_POWER, 0);
            ...
            {
                tp_log_info("Double click to light up the screen!");
                doze_status = DOZE_WAKEUP;
                input_report_key(ts->input_dev, KEY_POWER, 1);
                input_sync(ts->input_dev);
                input_report_key(ts->input_dev, KEY_POWER, 0);
    ...
    if (ret < 0)
    {
        tp_log_err("I2C transfer error. errno:%d\n ", ret);
        goto exit_work_func;
    }
    ...
    if(key_value || pre_key)
    {
        for (i = 0; i < GTP_MAX_KEY_NUM; i++)
        {
            ...
            for (ret = 0; ret < 4; ++ret)
            {
                ...
                {
                    tp_log_debug("Key: %s %s", key_names[ret], (key_value & (0x01 << i)) ? "Down" : "Up");
                    break;
                }
            }
        #endif
	...
	tp_log_debug("pre_touch:%02x, finger:%02x.", pre_touch, finger);
    ...
    if (pre_touch || touch_num)
    {
        ...
        tp_log_debug("id = %d,touch_index = 0x%x, pre_touch = 0x%x\n",id, touch_index,pre_touch);
...
exit_work_func:
    if(!ts->gtp_rawdiff_mode)
    {
        ...
        if (ret < 0)
        {
            tp_log_info("I2C write end_cmd error!");
        }
    }
    if (ts->use_irq)
    {
...
static s8 gtp_enter_doze(struct goodix_ts_data *ts)
{
    ...
    tp_log_debug("Entering doze mode.");
    while(retry++ < 5)
    {
        ...
        if (ret < 0)
        {
            tp_log_debug("failed to set doze flag into 0x8046, %d", retry);
            continue;
        }
        i2c_control_buf[0] = 0x80;
        ...
        if (ret > 0)
        {
            ...
            tp_log_info("GTP has been working in doze mode!");
            gtp_irq_enable(ts);
            return ret;
        }
        msleep(10);
...
static s8 gtp_enter_sleep(struct goodix_ts_data  *ts)
{
    ...
    GTP_DEBUG_FUNC();
    tp_log_debug("gtp_enter_sleep start ...line=%d",__LINE__);
#if GTP_COMPATIBLE_MODE
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
    ...
    while(retry++ < 5)
    {
        ...
        if (ret > 0)
        {
            tp_log_info("GTP enter sleep!");
            
            return ret;
        }
        msleep(10);
    }
    tp_log_err("GTP send sleep cmd failed.");
    return ret;
}
#endif
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data *ts)
{
    ...
    while(retry++ < 10)
    {
    tp_log_debug("wakeup retry=%d...line=%d",retry,__LINE__);
    #if GTP_SLIDE_WAKEUP
        ...
        {
            tp_log_debug("wakeup by slide/double-click, no reset guitar");
            doze_status = DOZE_DISABLED;
        #if GTP_ESD_PROTECT
        ...
        {
          tp_log_debug("wakeup chip_gt9xxs=%d...line=%d",chip_gt9xxs,__LINE__);
          gpio_direction_output(ts->pdata->irq_gpio, 1);
            msleep(5);
        }
    #endif
    ...
    tp_log_err("GTP wakeup sleep failed.");
    return ret;
}
#if GTP_DRIVER_SEND_CFG
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    if (ret == SUCCESS)
    {
        snprintf(touch_info,(sizeof(touch_info)-1),"goodix_gt915_boen.%c",opr_buf[0]);
...
static int gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	if (gpio_is_valid(pdata->irq_gpio)) {
		...
		if (ret) {
			tp_log_err("set_direction for irq gpio failed\n");
			goto free_irq_gpio;
		}
	} else {
	...
	if (gpio_is_valid(pdata->reset_gpio)) {
		...
		if (ret) {
			tp_log_err("set_direction for reset gpio failed\n");
...
static int gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	tp_log_debug("INT trigger type:%x, irq=%d", ts->int_trigger_type,
			ts->client->irq);
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "goodix,reset-gpio",&temp_val);
	if (ret)
	{
	...
	ret = of_property_read_u32(np, "goodix,irq-gpio",&temp_val);
	if (ret)
	{
 ...
 static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret < 0)
		...
		tp_log_err("%s:GTP works in interrupt mode.\n",__func__);
...
static void goodix_ts_resume(struct goodix_ts_data *ts)
{
	...
	if (ret < 0)
    {
        tp_log_err("GTP later resume failed.");
    }
#if (GTP_COMPATIBLE_MODE)
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
				 ...
				 unsigned long event, void *data)
{
	...
	tp_log_debug("fb_notifier_callback enter \n");
