...
extern int tp_get_open_enable_value(void);
/*
...
int  cyttsp4_get_panel_data_check(char **buf)
{
	...
	if (rc < 0) {
		...
		tp_log_err( "%s: Error on request exclusive r=%d\n",
				__func__, rc);
		goto cyttsp4_request_exclusive_fail;
	}
	...
	if(tp_get_open_enable_value())
		{
            ...
            tp_log_err("%s:begin to cyttsp4_check_open_data check\n",__func__);
			rc=cyttsp4_check_open_data(dev);
	        tp_log_info("%s:rc_open = %d\n", __func__, rc);
			if(rc < 0)
			{
...
cyttsp4_get_panel_data_show_err_release:
	if(need_output){
		cyttsp4_out_to_buf(rc, buf);
		need_output = false;
		tp_log_err("%s: raw data check fail\n",__func__);
		rc = cyttsp4_release_exclusive(dad->ttsp);
		if (rc < 0) {
			tp_log_err( "%s: Error on release exclusive r=%d\n",
					__func__, rc);
...
cyttsp4_request_exclusive_fail:
