...
static unsigned int tp_open_check_enable = 0;
...
int tp_get_open_enable_value(void)
{
    tp_log_info("%s: return tp_open_check_enable is %d\n", __func__, tp_open_check_enable);
    return tp_open_check_enable;
}
		...
		struct device_node *core_node)
{
	...
	rc = of_property_read_u32(core_node, "cy,open_check_enable", &value);
	if (!rc)
	{
	    tp_log_info("%s: get open_check_enable success!\n", __func__);
		tp_open_check_enable = value;
	}
	else
	{
		tp_log_info("%s: No open_check_enable dts info!\n", __func__);
		tp_open_check_enable = 0;
	}
