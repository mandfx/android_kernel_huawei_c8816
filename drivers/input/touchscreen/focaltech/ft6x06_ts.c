...
int focaltech_debug_mask = TP_ERR;
...
static int ft6x06_read_Touchdata(struct ft6x06_ts_data *data)
{
	...
	tp_log_info("in ft6x06_read_Touchdata\n");
	...
	tp_log_info("out ft6x06_read_Touchdata\n");
	return 0;
}
...
static int ft6x06_init_virtual_key(struct device_node *dev_node,
		const char *inp_dev_name, struct ft6x06_virtual_keys_data *virtual_keys_pdata)
{
	int rc = 0;
	char *name = NULL;
	u32 *data = NULL;
...
static void ft6x06_del_virtual_key(struct device_node *dev_node,
		const char *inp_dev_name, struct ft6x06_virtual_keys_data *virtual_keys_pdata)
{
	sysfs_remove_file(ft6x06_virtual_key_properties_kobj, &virtual_keys_pdata->kobj_attr.attr);
	kobject_del(ft6x06_virtual_key_properties_kobj);
	
	kfree(virtual_keys_pdata->data);
	virtual_keys_pdata->data = NULL;
	virtual_keys_pdata->size  = 0;
...
{
	ft6x06_del_virtual_key(dev_node,NULL,ft6x06_ts->ft6x06_core_pdata->virtual_keys_pdata);
...
static int ft6x06_register_virtual_key_device(struct device_node *dev_node,struct ft6x06_ts_data *ft6x06_ts)
{
	...
	/*init virtual keys */
	rc = ft6x06_init_virtual_key(dev_node,name,ft6x06_core_pdata->virtual_keys_pdata);
	if(rc)
	{
		tp_log_err("error:%s,line=%d,rc=%d\n", __func__, __LINE__,rc);
		rc = -EINVAL;
...
static char *ft6x06_get_touch_module_name(unsigned char  Vendor_ID)
{
	char *buf;
	switch(Vendor_ID)
	{
...
void ft6x06_set_app_info_touchpanel(struct ft6x06_ts_data *ft6x06_ts)
{
	...
	if(rc < 0)
		...
		tp_log_err("ft6x06_ts->uc_reg_value = %d\n",ft6x06_ts->fw_ver_reg_value);
	/*read Vendor ID*/
	rc = ft6x06_i2c_Read(ft6x06_ts->client, &vendor_id_addr, 1, &ft6x06_ts->vendor_id_value, 1);
	if(rc < 0)
		tp_log_err("error:%s,line=%d,rc=%d\n", __func__, __LINE__,rc);
	else 
		tp_log_err("ft6x06_ts->vendor_id_value = %d\n",ft6x06_ts->vendor_id_value);
	
	module_name = ft6x06_get_touch_module_name(ft6x06_ts->vendor_id_value);
	snprintf(touch_info, sizeof(touch_info),"Focaltech_Ft6x06_%s.%d",module_name,ft6x06_ts->fw_ver_reg_value);
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (rc < 0)
	{
		tp_log_err("error:%s,line=%d,rc=%d, i2c_address=0x%x\n", __func__, __LINE__,rc, client->addr);
		goto i2c_read_connected_fail;
	}
	else 
		tp_log_debug("%s,line=%d,rc=%d, i2c_address=0x%x\n", __func__, __LINE__,rc, client->addr);
